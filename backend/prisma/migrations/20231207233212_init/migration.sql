...
CREATE TABLE `naudotojai` (
    ...
    `fk_Administratoriusid_Administratorius` INTEGER NOT NULL,
    `fk_Darbuotojasid_Darbuotojas` INTEGER NOT NULL,

    UNIQUE INDEX `fk_Darbdavysid_Darbdavys`(`fk_Darbdavysid_Darbdavys`),
    UNIQUE INDEX `fk_Administratoriusid_Administratorius`(`fk_Administratoriusid_Administratorius`),
...
-- AddForeignKey
ALTER TABLE `naudotojai` ADD CONSTRAINT `naudotojai_id_Naudotojas_fkey` FOREIGN KEY (`id_Naudotojas`) REFERENCES `naudotoju_patiktukai`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
